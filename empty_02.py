...
class MongoDataLayer(BaseDataLayer):
    ...
    # ✅ UPDATED: delete_thread logic replaced using your "resolved" cascade logic (sessions removed, since this code has no sessions)
    async def delete_thread(self, thread_id: str) -> bool:
        ...
        Cascade delete:
          ...
          - steps
          - elements
          - feedback (by threadId + by forId using step ids)
        ...
        try:
            ...
            # Also gather legacy steps (thread_id)
            cursor_legacy = self.col_steps.find({"thread_id": thread_id}, {"id": 1})
            async for s in cursor_legacy:
                if s.get("id"):
                    step_ids.append(s["id"])
            ...
            fb2 = None
            if step_ids:
                fb2 = await self.col_feedback.delete_many({"forId": {"$in": list(set(step_ids))}})

            # Delete elements (threadId + legacy thread_id)
            el1 = await self.col_elements.delete_many({"threadId": thread_id})
            el2 = await self.col_elements.delete_many({"thread_id": thread_id})

            # Delete steps (threadId + legacy thread_id)
