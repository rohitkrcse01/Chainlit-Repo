...
def _thread_name_from_step_metadata(step: Dict[str, Any]) -> str:
    """
    FIX (as per your OLD code + your screenshot):
    Thread name MUST come from:
      step["metadata"]["chat_settings"]["chat_history"][0]["content"] (where role == "user")

    We intentionally do NOT use step["name"], step["threadName"], step["message"], etc.
    because in your system those can become userIdentifier.
    """
    meta = step.get("metadata")
    if not isinstance(meta, dict):
        return "Untitled"

    chat_settings = meta.get("chat_settings")
    if not isinstance(chat_settings, dict):
        return "Untitled"

    chat_history = chat_settings.get("chat_history")
...
class MongoDataLayer(BaseDataLayer):
    ...
    async def create_step(self, step_dict: Dict[str, Any]) -> str:
        ...
        FIX: thread.name is always taken from metadata.chat_settings.chat_history user content.
        """
        step = dict(step_dict)
        ...
        # Only create/update thread for user message steps
        ...
        if not existing:
            # ✅ THE FIX (matching your OLD design)
            thread_name = _thread_name_from_step_metadata(step)
    ...
    async def list_threads(self, pagination: Any, filters: Any) -> CLPaginatedResponse:
        """
        Same behavior as your old code style:
        filters.userId is expected to be a Mongo ObjectId string.
        """
    ...
    async def update_thread(
        ...
        """
        Keep existing behavior (upsert TRUE like your old code).
        We do NOT change name logic here; name is set at thread creation only from first user message.
        """
        try:
            patch: Dict[str, Any] = {"updated_at": _now()}
    ...
    async def delete_thread(self, thread_id: str) -> bool:
        """
        Your delete logic (kept correct):
          - threads
          - steps
          - elements
        ...
        try:
            ...
            # legacy cleanup (if any old docs exist)
